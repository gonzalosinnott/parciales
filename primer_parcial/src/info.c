...
typedef struct
{
	...
	char aux_status[LEN_STATUS];
	char aux_publicationNumber;
}ClientAuxiliar;
	...
	if(client_list !=NULL && client_len > 0 && publication_list !=NULL && publication_len > 0)
	{
		...
		else
		{
			for(int i = 0; i < client_len; i++)
			{
					...
					arrayAux[i].aux_isEmpty=client_list[i].client_isEmpty;
			...
			do
			{
				...
				{
					if(arrayAux[i].aux_publicationNumber  < arrayAux[i + 1].aux_publicationNumber)
					{
						bufferAux = arrayAux[i];
						arrayAux[i] = arrayAux[i + 1];
						arrayAux[i + 1] = bufferAux;
						sorted = FALSE;
					}
				}
			}while(sorted == FALSE);
			...
			for(int i=0;i< client_len ;i++)
			{
				if(arrayAux[i].aux_isEmpty == FALSE)
				{
					printf("| %-16s| %-16s| %-16s| %-4d | %-21d|\n",
							...
							arrayAux[i].aux_cuit,
							arrayAux[i].aux_id,
							arrayAux[i].aux_publicationNumber);
					printf("-------------------------------------------------------------------------------------\n");
					retorno = 0;
				}
			}
								...
								"\nOpcion:", "\nError.", &choosenOption, 3, 6, 1)==0)
			{
				switch(choosenOption)
				{
					...
					case 3:
						...
						break;
					case 4:
						if(info_clientWithMostActiveAds(client_list, client_len, publication_list, publication_len, publication_firstLoad)!=0)
						{
							printf("\nERROR. NO HAY PUBLICACIONES ACTIVAS.\n");
						}
						break;
					case 5:
						if(info_clientWithMostPausedAds(client_list, client_len, publication_list, publication_len, publication_firstLoad)!=0)
						{
							printf("\nERROR. NO HAY PUBLICACIONES PAUSADAS.\n");
						}
						break;
				}
				retorno =0;
...
/**
 * \brief info_clientWithMostActiveAds: Prints the information of the clients with the most ACTIVE publications
 * \param Client* client_list: Pointer to array of clients
 * \param int client_len: Clients Array length
 * \param Publication* publication_list: Pointer to array of publications
 * \param int publication_len: Publications Array length
		...
		else
		{
			for(int i=0;i< client_len ;i++)
			{
				...
				if(client_list[i].client_isEmpty == FALSE)
				{
					...
					for(int j=0;j< publication_len ;j++)
					{
						if(publication_list[j].publication_isEmpty == FALSE &&
						   publication_list[j].publication_idClient == idClient &&
						   strncmp(publication_list[j].publication_status,ACTIVE,LEN_CATEGORY)==0)
						{
							count++;
							if(count>=maxCount)
							{
								...
								strncpy(arrayAux[i].aux_lastName,client_list[i].client_lastName,LEN_NAME);
								strncpy(arrayAux[i].aux_cuit,client_list[i].client_cuit,LEN_FORMATEDCUIT);
								strcpy(arrayAux[i].aux_status,publication_list[j].publication_status);
								arrayAux[i].aux_publicationNumber = count;
								retorno = 0;
							}
						}
			...
			{
				printf("-------------------------------------------------------------------------------------\n");
				printf("|                          CLIENTES CON MAS PUBLICIDADES ACTIVAS                    |\n");
				printf("-------------------------------------------------------------------------------------\n");
				printf("| APELLIDO        | NOMBRE          | CUIT            |  ID  | CANTIDAD DE ANUNCIOS |\n");
				printf("-------------------------------------------------------------------------------------\n");
				for(int i=0;i< AUX_LEN  ;i++)
				{
					if(arrayAux[i].aux_isEmpty == FALSE &&
					   arrayAux[i].aux_publicationNumber == maxCount &&
					   strcmp(arrayAux[i].aux_status,ACTIVE)==0)
					{
						printf("| %-16s| %-16s| %-16s| %-4d | %-21d|\n",
								arrayAux[i].aux_lastName,
								arrayAux[i].aux_name,
...
/**
 * \brief info_clientWithMostPausedAds: Prints the information of the clients with the most PAUSED publications
 * \param Client* client_list: Pointer to array of clients
 * \param int client_len: Clients Array length
 * \param Publication* publication_list: Pointer to array of publications
 * \param int publication_len: Publications Array length
...
static int info_clientWithMostPausedAds(Client* client_list, int client_len, Publication* publication_list, int publication_len,int publication_firstLoad)
{
	...
	if(client_list !=NULL && client_len > 0 && publication_list !=NULL && publication_len > 0)
	{
		...
		else
		{
			for(int i=0;i< client_len ;i++)
			{
				...
				if(client_list[i].client_isEmpty == FALSE)
				{
					...
					for(int j=0;j< publication_len ;j++)
					{
						if(publication_list[j].publication_isEmpty == FALSE &&
						   publication_list[j].publication_idClient == idClient &&
						   strncmp(publication_list[j].publication_status,PAUSED,LEN_CATEGORY)==0)
						{
							count++;
							if(count>=maxCount)
							{
			...
			{
				printf("-------------------------------------------------------------------------------------\n");
				printf("|                          CLIENTES CON MAS PUBLICIDADES PAUSADAS                   |\n");
				printf("-------------------------------------------------------------------------------------\n");
				printf("| APELLIDO        | NOMBRE          | CUIT            |  ID  | CANTIDAD DE ANUNCIOS |\n");
				printf("-------------------------------------------------------------------------------------\n");
				for(int i=0;i< AUX_LEN  ;i++)
				{
					if(arrayAux[i].aux_isEmpty == FALSE &&
					   arrayAux[i].aux_publicationNumber == maxCount &&
					   strcmp(arrayAux[i].aux_status,PAUSED)==0)
